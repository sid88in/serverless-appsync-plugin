...
/**
 * Look up the deployed AppSync API's ARN by reading the CloudFormation stack's
 * resources (independent of the plugin) so the credential/region proof can
 * assert WHERE the API actually landed.
 */
export async function getDeployedApiArn(
  region: string,
  stackName: string,
): Promise<string | undefined> {
  const client = cfnClient(region);
  const { StackResources } = await client.send(
    new DescribeStackResourcesCommand({ StackName: stackName }),
  );
  return StackResources?.find(
