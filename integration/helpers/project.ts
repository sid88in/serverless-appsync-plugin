...
function buildServerlessYaml(opts: {
  ...
  lines.push(
    ...
    '      type: NONE',
    '',
    // Expose the deployed API ARN as a stack output so the credential/region
    // proof can read WHERE the API landed from `serverless info --verbose`
    // (a child process) instead of constructing an AWS SDK client inside the
    // Jest VM, which the v3 credential provider's dynamic import() breaks.
    'resources:',
    '  Outputs:',
    '    IntegrationApiArn:',
    '      Value:',
